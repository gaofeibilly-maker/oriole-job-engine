...
test("a clean public clone bootstraps eight reviewed sources and zero jobs", async () => {
  ...
  assert.equal(seedManifest.sources.length, 8);
  ...
  assert.equal(bootstrapped.imported, 8);
  assert.equal(bootstrapped.jobs.received, 0);
  assert.equal(state.jobs.length, 0);
  assert.equal(status.sourceCounts.approved, 8);
