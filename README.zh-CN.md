...
`main` 主代码树和全新克隆初始为**零岗位记录**。执行 `npm run init` 会导入 9 个明确审计过的公开来源种子——8 个企业自有招聘板（含字节跳动）和 1 个政府公共就业来源，让全新克隆可以立即拥有可运行的采集目标，同时不在种子中捆绑可能过期的岗位快照。已经部署的公开仓库可以把当前岗位和运行收据明确保存在独立的 `oriole-state` 分支；这份持久运行状态不属于全新克隆的初始种子。这不等于“搜索结果自动批准”：种子清单本身就是经过审核的信任决策，此后新发现的来源仍然必须探测和人工批准。
