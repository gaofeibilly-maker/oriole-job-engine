...
Expected result: every test passes, the nationwide smoke command returns the Hubei province entry with its prefecture-level children (including Wuhan), and the credential scan prints no findings. Public CI runs this exact complete command.
...
- Lever, Greenhouse, Ashby, JSON-LD, XML, RSS/Atom, and generic payload behavior;
...
- init succeeds without a bundled job snapshot and imports 8 audited public sources;
- status shows 8 approved sources and 0 bundled jobs before collection;
...
Verify that 15 tools are returned and that `huangque.review_source` is rejected unless MCP review was explicitly enabled. Repeat initialization with `2025-06-18` and `2025-03-26`; each successful response must return the exact requested protocol revision.
...
`providerConfiguration.baidu` must be `not_configured`. Ordinary keyword tasks must remain visible under `discoveryBacklog.blockedTasks` with reason `baidu_not_configured_or_not_selected`; they must not advance as completed. This does not block collection of the eight pre-approved sources or official-catalog discovery. Common Crawl is runnable only for query-plan tasks containing a controlled `site:` pattern.
...
The first command must print nothing. Confirm that `.openai/`, `.huangque/`, app/worker/database code, live snapshots, raw responses, `.env`, and credentials are absent from the public release.
